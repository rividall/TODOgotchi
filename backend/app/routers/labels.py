...
async def _get_workspace_label(db: AsyncSession, label_id: int, user: User) -> Label:
    result = await db.execute(select(Label).where(Label.id == label_id))
    label = result.scalar_one_or_none()
    if label is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Label not found")
    if label.workspace_id != user.workspace_id:
...
async def _get_workspace_poring(db: AsyncSession, poring_id: int, user: User) -> Poring:
    ...
    if poring.workspace_id != user.workspace_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not in your workspace")
    return poring
...
async def list_labels(
    ...
    if current_user.workspace_id is None:
        return []
    result = await db.execute(
        select(Label).where(Label.workspace_id == current_user.workspace_id).order_by(Label.name)
    )
...
async def create_label(
    ...
    existing = await db.execute(
        select(Label).where(
            Label.workspace_id == current_user.workspace_id,
    ...
    label = Label(name=payload.name, color=payload.color, workspace_id=current_user.workspace_id)
    db.add(label)
    await db.commit()
...
async def attach_label(
    ...
    label = await _get_workspace_label(db, label_id, current_user)
