...
> **Note:** The schema below reflects the target standard. `workspaces` and `user_workspaces` tables may not exist yet — they will be added during the admin integration migration.
...
| `hashed_password` | varchar(255) | bcrypt — never exposed by API |
| `workspace_id` | integer FK → workspaces.id | active workspace, nullable — to be added |
| `created_at` | timestamptz | auto-set |

### `user_workspaces` (junction)
| Column | Type | Notes |
|--------|------|-------|
| `user_id` | integer FK → users.id | composite PK, CASCADE on delete |
| `workspace_id` | integer FK → workspaces.id | composite PK, CASCADE on delete |
...
| `name` | varchar(255) | |
| `stage` | integer | maturation stage 1–5 |
| `xp` | integer | current XP |
| `status` | varchar(20) | `alive` / `completed` / `dead` |
| `created_at` | timestamptz | |
...
|--------|------|-------|
| `id` | integer PK | |
| `poring_id` | integer FK → porings.id | CASCADE on delete |
| `text` | varchar(500) | |
...
| `reason` | varchar(100) | e.g. `checklist_item_done` |
| `created_at` | timestamptz | |
...
All admin endpoints are under `/admin/` and require:
```
X-Admin-Key: <ADMIN_API_KEY from .env>
```

Generate a key: `openssl rand -hex 32`. Add to `.env` as `ADMIN_API_KEY=...`, then rebuild.

| Method   | Endpoint | Description |
|----------|----------|-------------|
| `GET`    | `/admin/users` | List all users with workspace memberships |
| `POST`   | `/admin/users` | Create user |
| `DELETE` | `/admin/users/:id` | Delete user and all their porings |
...
```
`workspace_id` is optional — omit to auto-create a personal workspace. Password is bcrypt-hashed server-side.

**Delete user:** also deletes all their porings, checklist items, labels, and XP events (FK cascade).
...
| Method   | Endpoint | Description |
|----------|----------|-------------|
| `GET`    | `/admin/workspaces` | List all workspaces with members |
| `POST`   | `/admin/workspaces` | Create empty workspace |
| `DELETE` | `/admin/workspaces/:id` | Delete workspace |
| `POST`   | `/admin/workspaces/:id/members` | Add member by email |
| `DELETE` | `/admin/workspaces/:id/members/:user_id` | Remove member |

**Typical workflow — add a new user:**
```bash
# 1. Create user (personal workspace auto-created)
curl -X POST https://porings.buenalynch.com/admin/users \
  -H "X-Admin-Key: your_key" \
  -H "Content-Type: application/json" \
  -d '{"email":"alice@example.com","username":"alice","password":"temp_pass"}'
```
