...
- [X] **[UI] Landing world picker** -- Top-right dropdown on the public landing page to switch the showcased world (Forest / Space / Graveyard) with the same 1-big + 3-small idle scene per world. Initial implementation in [LandingDino.tsx](../frontend/src/field/LandingDino.tsx) + [LandingPage.tsx](../frontend/src/pages/LandingPage.tsx) was wired but visuals were broken (creature scales/positions per world, decoration safe-radius around centered creature). Picker hidden in LandingPage for now — revisit and tune scales/positions, then re-expose the `<select>` and dynamic `field-stage-world-${world}` class. The `LandingDino` `world` prop already accepts all WorldIds.
...
- [x] **[Auth] Guest access** -- DONE (2026-04-29). In-memory guest store with 4 real preset critters; full XP/checklist/label interactions; sessionStorage-backed flag survives HMR. No backend endpoint needed — entirely client-side.
- [x] **[INFO] Add feedback input for third parties** -- DONE (2026-04-29). Public `POST/GET /api/v1/feedback`; `FeedbackModal` on guest field; admin moderation at `/admin`.
...
- [ ] **[Server] Add Uptime Kuma monitor** -- Add HTTP check for `todogotchi.buenalynch.com` at uptime.buenalynch.com.
- [x] **[Tunnel] Update cloudflared ingress** -- DONE (2026-04-29). Ingress rule added, cloudflared restarted, verified live.
...
- [X] **[QA] Browser smoke test of Phase 1** -- Run `make dev` and walk register → create poring → edit description → confirm tier flips at 10 XP.
- [X] **[QA] Browser smoke test of Phase 2** -- On top of the Phase 1 walk: add/check/uncheck a checklist item and confirm XP bumps by 3 + 5; toggle completed again and confirm no extra XP; create a new label via the picker and confirm it attaches + shows up on other porings' pickers; detach and confirm XP is retained.
- [X] **[QA] Browser smoke test of Phase 3** -- Feed a poring to 60+ XP; confirm ripe glow + "Act on this poring" CTA in the panel; open ActModal and confirm one action type is selectable; confirm cancel → panel still open; confirm "Pop it" triggers the star burst; confirm the poring moves to the CompletedDrawer; click it there and confirm the panel opens read-only; confirm the backend rejects edits.
...
- [x] **[Art/Tier 1] Kenney Nature Kit environment** -- DONE (2026-04-25). Multi-world decoration system with Forest, Forest_ISO, Forest_retro, Space, Graveyard worlds. Space has rotating asteroids + ship creatures. Graveyard has 5 character variants + paired trees + graves.
- [X] **[Phase 5] Multi-world / biomes** -- Architectural v2: labels gain a `world` field, porings render as per-world creatures (forest/city/graveyard/dungeon/space/ocean...). Full analysis + rollout plan in [docs/research/multi-world-pipeline-analysis.md](research/multi-world-pipeline-analysis.md). **Do NOT start until the forest looks visibly great via the single-world art pipeline** — prereq for proving the abstraction.
